...
class NPOSTrainer:
    def __init__(
        ...
        if self.config.num_gpus > 1:
            params = [
                ...
                {
                    "params": net.module.mlp.parameters(),
                    "lr": config.optimizer.lr * config.optimizer.mlp_decay_rate,
                },
            ]
        else:
            params = [
    ...
    def train_epoch(self, epoch_idx):
        ...
        data_dict = torch.zeros(
            self.n_cls, self.sample_number, self.penultimate_dim
        ).cuda()
        ...
        for train_step in tqdm(
            ...
            if (
                sum_temp == self.n_cls * self.sample_number
                ...
                for index in range(len(target)):
                    ...
                    data_dict[dict_key] = torch.cat(
                        (
                            ...
                            penultimate[index].detach().view(1, -1),
                        ),
                        0,
                    )
            elif (
                ...
                if len(ood_samples) != 0:
                    ...
                    input_for_lr = torch.cat(
                        (energy_score_for_fg, energy_score_for_bg), 0
                    ).squeeze()
                    labels_for_lr = torch.cat(
                        (
                            torch.ones(len(energy_score_for_fg)).cuda(),
                            torch.zeros(len(energy_score_for_bg)).cuda(),
                        ),
                        -1,
                    )
                ...
                for index in range(len(target)):
                    ...
                    if self.number_dict[dict_key] < self.sample_number:
                        data_dict[dict_key][self.number_dict[dict_key]] = penultimate[
                            index
                        ].detach()
            ...
            comp_loss = self.criterion_comp(
                normed_features, self.criterion_disp.prototypes, target
            )
    ...
    def forward(self, features, labels):
        ...
        for j in range(len(features)):
            prototypes[labels[j].item()] = F.normalize(
                prototypes[labels[j].item()] * self.proto_m
                + features[j] * (1 - self.proto_m),
                dim=0,
            )
        ...
        logits_mask = torch.scatter(
            torch.ones_like(mask), 1, torch.arange(num_cls).view(-1, 1).cuda(), 0
        )
...
def generate_outliers(
    ...
    rand_ind = np.random.choice(
        normed_data.shape[0], int(normed_data.shape[0] * sampling_ratio), replace=False
    )
    index = input_index
    ...
    point = KNN_dis_search_distance(
        negative_sample_list, index, K, ID_points_num, length, depth
    )
