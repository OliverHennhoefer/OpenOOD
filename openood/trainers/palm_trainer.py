...
class PALMTrainer(nn.Module):
    def __init__(
        ...
        self.register_buffer("protos", torch.rand(self.n_protos, self.feat_dim).cuda())
    ...
    def mle_loss(self, features, targets):
        ...
        contrast_labels = (
            torch.arange(self.num_classes).repeat(self.cache_size).view(-1, 1).cuda()
        )
        ...
        if self.k > 0:
            ...
            topk_mask = torch.scatter(
                torch.zeros_like(update_mask), 1, topk_idx, 1
            ).cuda()
            update_mask = F.normalize(
                F.normalize(topk_mask * update_mask, dim=1, p=1), dim=0, p=1
            )
        # original
        else:
