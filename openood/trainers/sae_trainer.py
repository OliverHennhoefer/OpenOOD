...
class SAETrainer:
    ...
    def setup(self):
        ...
        label_all = []
        # collect features
        for batch in tqdm(self.train_loader, desc="Compute GMM Stats [Collecting]"):
        ...
        transform_matrix = reduce_feature_dim(
            feature_all, label_all, self.trainer_args.reduce_dim
        )
        feature_all = np.dot(feature_all, transform_matrix)
        # GMM estimation
        gm = GaussianMixture(
            n_components=self.trainer_args.num_clusters,
            random_state=0,
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(
            ...
            prob_id = compute_single_GMM_score(
                self.net,
                data,
                self.feature_mean,
                self.feature_prec,
            ...
            prob_ood = compute_single_GMM_score(
                self.net,
                data_mix,
                self.feature_mean,
                self.feature_prec,
            ...
            # loss
            loss = (
                self.trainer_args.loss_weight[0] * loss_clsstd
