...
class CIDERTrainer:
    def __init__(
        ...
        self.criterion_comp = CompLoss(
            config.dataset.num_classes, temperature=config.trainer.trainer_args.temp
        ).cuda()
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(
            ...
            comp_loss = self.criterion_comp(
                features, self.criterion_dis.prototypes, target
            )
...
class CompLoss(nn.Module):
    ...
    def forward(self, features, prototypes, labels):
        ...
        feat_dot_prototype = torch.div(
            torch.matmul(features, prototypes.T), self.temperature
        )
