    ...
    def predict(
        ...
        try:
            ...
            if current_device != self.device:
                warnings.warn(
                    f"Model device changed from {self.device} to {current_device}. Updating scanner device.",
                    UserWarning,
                )
        ...
        hooks_needed = bool(
            self.target_modules
        )  # Hooks are always needed if targets exist
        ...
        for batch in iterator:
            ...
            if not isinstance(batch, (list, tuple)) or not batch:
                warnings.warn(
                    f"Skipping batch {batch_num}: Expected list or tuple, got {type(batch)}.",
                    UserWarning,
                )
                continue
            ...
            if include_image_id and image_ids_batch is not None:
                if isinstance(image_ids_batch, torch.Tensor):
                    ...
                    )  # Assume CPU tensor or move if needed
                else:
            ...
            for i in range(batch_size):  # Iterate through each sample in the batch
                ...
                if labels_list is not None:
                    try:
                        ...
                        warnings.warn(
                            f"Label index {i} out of bounds for batch {batch_num}.",
                            UserWarning,
                        )
                ...
                if image_ids_list is not None:
                    try:
                        ...
                        warnings.warn(
                            f"Image ID index {i} out of bounds for batch {batch_num}.",
                            UserWarning,
                        )
                    ...
                    try:
                        ...
                        if i < batch_tensor.shape[0]:
                            sample_item = batch_tensor[
                                i
                            ]  # Extract data for the i-th sample (on CPU)

                            # --- Apply GAP or store vector ---
                            if (
                                sample_item.ndim >= 3
                            ):  # Input to ConvNd (e.g., C, H, W or C, D, H, W)
                                # Calculate GAP: Mean over all spatial dimensions (starting from dim 1)
                                spatial_dims = tuple(range(1, sample_item.ndim))
                                channel_means_tensor = sample_item.mean(
                                    dim=spatial_dims
                                )
                                # Store the resulting 1D tensor (length C) as a list
                                sample_data[f"{layer_name}_pre_activation_avg"] = (
                                    channel_means_tensor.tolist()
                                )

                            elif (
                                sample_item.ndim == 1
                            ):  # Input to Linear (e.g., Features)
                                # Store the 1D tensor as a list
                                sample_data[f"{layer_name}_pre_activation"] = (
                                    sample_item.tolist()
                                )

                            elif (
                                sample_item.ndim == 0
                            ):  # Scalar input (unlikely but possible)
                                sample_data[f"{layer_name}_pre_activation"] = (
                                    sample_item.item()
                                )
                                ...
                                sample_item.ndim == 2
                            ):  # E.g. input (Batch, SeqLen) to Conv1d? or (Batch, Features) to Linear?
                                ...
                                warnings.warn(
                                    ...
                                    f"and storing as '{layer_name}_pre_activation_avg'.",
                                    UserWarning,
                                )
                                mean_tensor = sample_item.mean(
                                    dim=-1
                                )  # Mean over last dimension
                                sample_data[f"{layer_name}_pre_activation_avg"] = (
                                    mean_tensor.tolist()
                                    if mean_tensor.ndim > 0
                                    else mean_tensor.item()
                                )
                                ...
                                warnings.warn(
                                    ...
                                    "Flattening and storing as list.",
                                    UserWarning,
                                )
                                sample_data[f"{layer_name}_pre_activation"] = (
                                    sample_item.flatten().tolist()
                                )

                        else:  # Index out of bounds for tensor
                            warnings.warn(
                                f"Index {i} out of bounds for pre-activation tensor key '{key}' "
                                f"(shape {batch_tensor.shape}) in batch {batch_num}.",
                                UserWarning,
                            )
                        ...
                        sample_data[key.replace("_pre_activation", "_error")] = str(
                            e
                        )  # Or just None
        ...
        print(
            f"Converting {len(self.extracted_data_list)} extracted samples to Polars DataFrame..."
        )
        try:
        ...
        ordered_cols = []
        if "image_id" in cols:
            ordered_cols.append("image_id")
        if "label" in cols:
            ordered_cols.append("label")

        # Add pre-activation columns, sorted alphabetically
        activation_cols = sorted(
            [c for c in cols if "_pre_activation" in c or "_error" in c]
        )
        ...
        try:
            ...
            if len(set(ordered_cols)) != len(ordered_cols):
                warnings.warn(
                    "Duplicate column names detected during reordering. Using default order.",
                    UserWarning,
                )
                return results_df
