...
class ResidualPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        with torch.no_grad():
            ...
            feature_id_train = []
            for batch in tqdm(
                id_loader_dict["val"], desc="Eval: ", position=0, leave=True
            ):
            ...
            feature_id_val = []
            for batch in tqdm(
                id_loader_dict["test"], desc="Eval: ", position=0, leave=True
            ):
                data = batch["data"].cuda()
                data = data.float()
                _, feature = net(data, return_feature=True)
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        score_ood = -norm(np.matmul(feature_ood.cpu() - self.u, self.NS), axis=-1)
