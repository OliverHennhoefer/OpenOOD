...
class MDSEnsemblePostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        for layer_index in range(self.num_layer):
            ...
            pred, score = compute_Mahalanobis_score(
                net,
...
@torch.no_grad()
def get_MDS_stat(model, train_loader, num_classes, feature_type_list, reduce_dim_list):
    ...
    feature_class = [[None for x in range(num_classes)] for y in range(num_layer)]
    ...
    feature_mean_list = [
        [np.mean(feature_by_class, axis=0) for feature_by_class in feature_by_layer]
...
def compute_Mahalanobis_score(
    ...
    out_features = process_feature_type(
        out_features[layer_index], feature_type_list[layer_index]
    )
    ...
    )  # updated input data with perturbation
