    ...
    def save_model(self, net, val_metrics):
        ...
        if self.config.recorder.save_all_models:
            save_pth = os.path.join(
                self.save_dir, "epoch-{}_NetF.ckpt".format(epoch_idx)
            )
        ...
        if val_metrics["acc"] >= self.best_acc:
            ...
            torch.save(netF_wts, os.path.join(self.output_dir, "best_NetF.ckpt"))
            torch.save(
                criterion_wts, os.path.join(self.output_dir, "best_criterion.ckpt")
            )
