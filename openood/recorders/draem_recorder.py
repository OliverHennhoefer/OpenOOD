...
class DRAEMRecorder(ADRecorder):
    def __init__(self, config) -> None:
        ...
        self.run_name = (
            "draem_test_"
            + str(self.config.optimizer.lr)
            + "_"
            + str(self.config.optimizer.num_epochs)
            + "_bs"
            + str(self.config.dataset.train.batch_size)
            + "_"
    ...
    def save_model(self, net, test_metrics):
        if self.config.recorder.save_all_models:
            ...
            torch.save(net["discriminative"].state_dict(), save_pth + "_seg.ckpt")
