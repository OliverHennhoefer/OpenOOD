...
class OpenGanRecorder(BaseRecorder):
    ...
    def report(self, train_metrics, val_metrics):
        print(
            "Epoch [{:03d}/{:03d}] | Time {:5d}s | Loss_G: {:.4f} | "
    ...
    def save_model(self, net, val_metrics):
        ...
        if self.config.recorder.save_all_models:
            save_pth = os.path.join(
                self.save_dir, "epoch-{}_GNet.ckpt".format(epoch_idx)
            )
            torch.save(netG_wts, save_pth)
            save_pth = os.path.join(
                self.save_dir, "epoch-{}_DNet.ckpt".format(epoch_idx)
            )
        ...
        if val_metrics["auroc"] >= self.best_val_auroc:
            ...
            torch.save(netG_wts, os.path.join(self.output_dir, "best_GNet.ckpt"))
            torch.save(netD_wts, os.path.join(self.output_dir, "best_DNet.ckpt"))
