...
class KdadRecorder:
    ...
    def report(self, train_metrics, test_metrics):
        print(
            "epoch [{}],time:{:5d}s,loss:{:.4f},roc_auc:{:.2f}".format(
                train_metrics["epoch_idx"],
                int(time.time() - self.begin_time),
                train_metrics["epoch_loss"],
    ...
    def summary(self):
        print(
            "Training Completed! "
            "Best Roc_auc: {:.2f}%,"
            "at epoch {:d}".format(100 * self.best_roc_auc, self.best_epoch_idx),
            flush=True,
        )
