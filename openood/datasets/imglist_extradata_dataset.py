...
class ImglistExtraDataDataset(BaseDataset):
    def __init__(
        ...
        **kwargs
        ...
        self.extra_num = int(len(extra_labels) * extra_percent / 100.0)
    ...
    def __iter__(self):
        ...
        while batch_counter < self.num_batches:
            real_batch = real_inds_shuffled[
                real_offset : min(
                    real_offset + self.real_batch_size, len(real_inds_shuffled)
                )
            ]
            ...
            syn_batch = syn_inds_shuffled[
                syn_offset : min(
                    syn_offset + self.syn_batch_size, len(syn_inds_shuffled)
                )
            ]
