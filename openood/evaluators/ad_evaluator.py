...
class ADEvaluator:
    ...
    def eval_ood(
        self, net, id_data_loader, ood_data_loaders, postprocessor, epoch_idx: int = -1
    ):
        with torch.no_grad():
            if type(net) is dict:
            ...
            auroc = self.get_auroc(
                net, id_data_loader["test"], ood_data_loaders["val"], postprocessor
            )
            metrics = {
                "epoch_idx": epoch_idx,
                "image_auroc": auroc,
    ...
    def report(self, test_metrics):
        ...
        print(
            ...
            "AUC Image: {:.2f} \n"
            "==============================".format(
                self.config.dataset.name, 100.0 * test_metrics["image_auroc"]
            ),
            flush=True,
        )
