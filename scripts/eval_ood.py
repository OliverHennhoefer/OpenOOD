...
parser.add_argument("--wrapper-net", type=str, default=None, choices=["ASCOODNet"])
...
for subfolder in sorted(glob(os.path.join(root, "s*"))):
    ...
    if os.path.isfile(
        os.path.join(subfolder, "postprocessors", f"{postprocessor_name}.pkl")
    ):
        with open(
            os.path.join(subfolder, "postprocessors", f"{postprocessor_name}.pkl"), "rb"
        ) as f:
    ...
    if postprocessor_name == "conf_branch":
        net = ConfBranchNet(
            backbone=model_arch(num_classes=num_classes), num_classes=num_classes
        )
        ...
        backbone = model_arch(num_classes=num_classes)
        net = UDGNet(backbone=backbone, num_classes=num_classes, num_clusters=1000)
    elif postprocessor_name in ["cider", "reweightood"]:
        backbone = model_arch(num_classes=num_classes)
        net = CIDERNet(backbone, head="mlp", feat_dim=128, num_classes=num_classes)
    elif postprocessor_name == "npos":
        backbone = model_arch(num_classes=num_classes)
        net = NPOSNet(backbone, head="mlp", feat_dim=128, num_classes=num_classes)
    elif postprocessor_name == "palm":
        backbone = model_arch(num_classes=num_classes)
        net = PALMNet(backbone, head="mlp", feat_dim=128, num_classes=num_classes)
    ...
    if os.path.isfile(os.path.join(subfolder, "scores", f"{postprocessor_name}.pkl")):
        with open(
            os.path.join(subfolder, "scores", f"{postprocessor_name}.pkl"), "rb"
        ) as f:
    ...
    if (
        hasattr(evaluator.postprocessor, "setup_flag")
        ...
        if not os.path.isfile(os.path.join(pp_save_root, f"{postprocessor_name}.pkl")):
            with open(
                os.path.join(pp_save_root, f"{postprocessor_name}.pkl"), "wb"
            ) as f:
    ...
    if args.save_score:
        ...
        with open(
            os.path.join(score_save_root, f"{postprocessor_name}.pkl"), "wb"
        ) as f:
...
for i in range(len(metrics_mean)):
    ...
    for j in range(metrics_mean.shape[1]):
        temp.append(
            "{:.2f} \u00b1 {:.2f}".format(metrics_mean[i, j], metrics_std[i, j])
        )
