...
parser.add_argument(
    "--arch", default="resnet50", choices=["resnet50", "swin-t", "vit-b-16", "regnet"]
)
...
if not args.tvs_pretrained:
    ...
    root = os.path.join(
        ROOT_DIR, "results", f"imagenet_{args.arch}_tvsv{args.tvs_version}_base_default"
    )
...
if os.path.isfile(os.path.join(root, "postprocessors", f"{postprocessor_name}.pkl")):
    with open(
        os.path.join(root, "postprocessors", f"{postprocessor_name}.pkl"), "rb"
    ) as f:
...
if args.tvs_pretrained:
    if args.arch == "resnet50":
        ...
        net = RegNet_Y_16GF()
        weights = eval(f"RegNet_Y_16GF_Weights.IMAGENET1K_SWAG_E2E_V{args.tvs_version}")
        net.load_state_dict(load_state_dict_from_url(weights.url))
        preprocessor = weights.transforms()
    else:
    ...
    if args.arch == "resnet50":
        if postprocessor_name == "conf_branch":
            ...
            backbone = ResNet50()
            net = GodinNet(
                backbone=backbone, feature_size=backbone.feature_size, num_classes=1000
            )
        elif postprocessor_name == "rotpred":
            net = RotNet(backbone=ResNet50(), num_classes=1000)
        elif postprocessor_name in ["cider", "reweightood"]:
            net = CIDERNet(
                backbone=ResNet50(), head="mlp", feat_dim=128, num_classes=1000
            )
...
if os.path.isfile(os.path.join(root, "scores", f"{postprocessor_name}.pkl")):
    with open(os.path.join(root, "scores", f"{postprocessor_name}.pkl"), "rb") as f:
...
if (
    ...
    if not os.path.isfile(os.path.join(pp_save_root, f"{postprocessor_name}.pkl")):
        with open(os.path.join(pp_save_root, f"{postprocessor_name}.pkl"), "wb") as f:
...
if args.save_csv:
    ...
    if not os.path.isfile(os.path.join(saving_root, f"{postprocessor_name}.csv")):
        metrics.to_csv(
...
if args.save_score:
    ...
    with open(os.path.join(score_save_root, f"{postprocessor_name}.pkl"), "wb") as f:
