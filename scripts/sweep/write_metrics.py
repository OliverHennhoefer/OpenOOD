...
def write_metric(args, folder_list, save_line_dict, benchmark_dict):
    ...
    for metric in args.metrics:
        if metric == "ood":
            for benchmark in set(args.benchmarks) & set(benchmark_dict[metric]):
                args_list = make_args_list(
                    [benchmark], args.methods, ["ood"], benchmark_dict
                )
                sub_form_content = []
                for key_param in args_list:
                    ...
                    with open(
                        os.path.join(args.output_dir, target_folder, "ood.csv"), "r"
                    ) as f:
                        lines = f.readlines()[save_line_dict[key_param[-1]] :]
                    sub_line_content = {}
                    sub_line_content["method/{}".format(args.metric2save)] = key_param[
                        1
                    ]
                    for line in lines:
                        split = line.split(",")
                        ...
                        for metric in save_list:
                            content = (
                                content + "{:.2f}".format(float(split[metric])) + " / "
                            )
                        else:
                            content = content[:-3]
                ...
                csv_path = os.path.join(
                    args.output_dir, "{}_ood.csv".format(key_param[0])
                )
            ...
            for method in args.methods:
                args_list = make_args_list(
                    args.benchmarks, [method], ["osr"], benchmark_dict
                )
                sub_line_content = {}
                ...
                for key_param in args_list:
                    ...
                    for i, folder in enumerate(target_folder):
                        with open(
                            os.path.join(args.output_dir, folder, "ood.csv"), "r"
                        ) as f:
                            lines = f.readlines()[save_line_dict[key_param[-1]] :]
                        for line in lines:
                            split = line.split(",")
            ...
            with open(csv_path, "w", newline="") as csvfile:
                fieldnames = order_fieldnames(list(sub_form_content[0].keys()), args)
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writeheader()
                for sub_line_content in sub_form_content:
                    writer.writerow(sub_line_content)
...
def write_total(
    args, folder_list, save_line_dict, benchmark_dict, main_content_extract_dict
):
    main_form_content = []
    for method in args.methods:
        main_line_content = {}
        for metric in args.metrics:
            args_list = make_args_list(
                args.benchmarks, [method], [metric], benchmark_dict
            )
            for key_param in args_list:
                ...
                if metric == "ood":
                    ...
                    for line in lines:
                        if (
                            line.split(",")[0]
                            in main_content_extract_dict[key_param[-1]]
                        ):

                            # take auroc only
                            content = (
                                content
                                + "{:.2f}".format(float(line.split(",")[2]))
                                + " / "
                            )
                    else:
                        content = content[:-3]
