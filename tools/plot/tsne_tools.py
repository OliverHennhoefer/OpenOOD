...
def tsne_compute(x, n_components=50):
    ...
    print(
        "TSNE Computation Duration: {:0>2}:{:0>2}:{:05.2f}".format(
...
for idx, dataset in enumerate(dataset_list):
    ...
    index_select = np.random.choice(
        index_list, int(sample_rate * num_samples), replace=False
    )
...
highfeat_list, featstat_list, index_list = (
    np.array(highfeat_list),
    np.array(featstat_list),
    np.array(idx_list),
)
